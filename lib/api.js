 ...
 * Sends a DELETE request to: /api/collections/COLLECTION_ID?ownerId=XYZ
 */
export async function deleteCollection(collectionId, ownerId) {
  if (!collectionId || !ownerId) {
    throw new Error('collectionId and ownerId are required to delete a collection');
  }

  return apiFetch(`/api/collections/${encodeURIComponent(collectionId)}?ownerId=${encodeURIComponent(ownerId)}`, {
    method: 'DELETE',
  });
}
